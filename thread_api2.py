...
def process_point_item(data, idx, output_file):
    ...
    if 'source' in data.keys():
        ...
        meta = {'correct':correct, 'ans':ans, 'answer': answer,'question': question,  'labeled_solution': label_response}
    with write_lock:
...
def generate(dataset, output_file):

    existed = set() # 断点续传,避免重复生成
...
def sample(a_file, b_file, source):
    data_ls_1 = []
    question_Set = set()
    with open(a_file, 'r',encoding='utf-8') as f:
        for line in f:
            data = json.loads(line)
            if data['question'] not in question_Set and data['source'] == source:
                data_ls_1.append(data)
                question_Set.add(data['question'])
            if len(question_Set) == 100:
                break
    data_ls_2 = []
    with open(b_file, 'r',encoding='utf-8') as f:
        for line in f:
            ...
            if data['question'] in question_Set:
                data_ls_2.append(data)
            
    print(len(data_ls_1),len(data_ls_2))
    return data_ls_1, data_ls_2


# a_file = '7b-initial-new/7b-grpo-initial-step50-hf_test.jsonl'
# b_file = "7b-grpo-base/7b-grpo-base_gsm8k_test.jsonl"
# dataset1,dataset2 = sample(a_file,b_file,'gsm8k')

# # output_file1 = '7b-initial-new/label-7b-grpo-initial-step50-hf_gsm8k.jsonl'
# # generate(dataset1,output_file1)

# output_file2 = "7b-grpo-base/label-7b-grpo-base_gsm8k_test.jsonl"
# generate(dataset2,output_file2)

############################################
a_file = '7b-initial-new/7b-grpo-initial-step50-hf_test.jsonl'
b_file = "7b-grpo-base/7b-grpo-base_math500.jsonl"
dataset1,dataset2 = sample(a_file,b_file,'math500')

output_file1 = '7b-initial-new/label-7b-grpo-initial-step50-hf_math500.jsonl'
generate(dataset1,output_file1)

output_file2 = "7b-grpo-base/label-7b-grpo-base_math500.jsonl"
generate(dataset2,output_file2)